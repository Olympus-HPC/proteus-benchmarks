...
static __device__ __attribute__((always_inline)) void atomic_add( float *address, float value )
{
  atomicAdd( address, value );
}

static __device__ __attribute__((always_inline)) void atomic_add( double *address, double value )
{
...
#if 0
static __device__ __attribute__((always_inline)) int bfe( int src, int num_bits ) 
{
  unsigned mask;
...
static __device__ __attribute__((always_inline)) int bfind( int src ) 
{
  int msb;
...
static __device__ __attribute__((always_inline)) int bfind( unsigned long long src ) 
{
  int msb;
...
static __device__ __attribute__((always_inline)) unsigned long long brev( unsigned long long src ) 
{
...
static __device__ __attribute__((always_inline)) int lane_id() 
{
  int id;
...
static __device__ __attribute__((always_inline)) int lane_mask_lt() 
{
  int mask;
...
static __device__ __attribute__((always_inline)) int warp_id() 
{
...
{ 
  template< typename T >
  static __device__ __attribute__((always_inline)) T load( const T *ptr ) { return *ptr; }
};

template<>
struct Ld<LD_CG> 
{ 
  static __device__ __attribute__((always_inline)) int load( const int *ptr ) 
  { 
    int ret; 
  ...
  static __device__ __attribute__((always_inline)) float load( const float *ptr ) 
  { 
    float ret; 
  ...
  static __device__ __attribute__((always_inline)) double load( const double *ptr ) 
  { 
    double ret; 
...
{ 
  template< typename T >
  static __device__ __attribute__((always_inline)) T load( const T *ptr ) { return __ldg( ptr ); }
};
...
static __device__ __attribute__((always_inline)) void load_vec2( float (&u)[2], const float *ptr )
{
  asm( "ld.global.cg.f32.v2 {%0, %1}, [%2];" : "=f"(u[0]), "=f"(u[1]) : __PTR(ptr) );
}

static __device__ __attribute__((always_inline)) void load_vec2( double (&u)[2], const double *ptr )
{
  asm( "ld.global.cg.f64.v2 {%0, %1}, [%2];" : "=d"(u[0]), "=d"(u[1]) : __PTR(ptr) );
}

static __device__ __attribute__((always_inline)) void load_vec4( float (&u)[4], const float *ptr )
{
  asm( "ld.global.cg.f32.v4 {%0, %1, %2, %3}, [%4];" : "=f"(u[0]), "=f"(u[1]), "=f"(u[2]), "=f"(u[3]) : __PTR(ptr) );
}

static __device__ __attribute__((always_inline)) void load_vec4( double (&u)[4], const double *ptr )
{
...
// Shuffle.
// ====================================================================================================================
static __device__ __attribute__((always_inline)) float shfl( float r, int lane, int warp_size )
{
#if __CUDA_ARCH__ >= 700
  return __shfl_sync(FULL_MASK, r, lane , warp_size);
#elif __CUDA_ARCH__ >= 300 || __HIP_DEVICE_COMPILE__
...
static __device__ __attribute__((always_inline)) double shfl( double r, int lane, int warp_size )
{
#if __CUDA_ARCH__ >= 700
...
static __device__ __attribute__((always_inline)) float shfl_xor( float r, int mask, int warp_size )
{
#if __CUDA_ARCH__ >= 700
...
static __device__ __attribute__((always_inline)) double shfl_xor( double r, int mask, int warp_size )
{
#if __CUDA_ARCH__ >= 700
...
static __device__ __attribute__((always_inline)) float shfl_down( float r, int offset )
{
#if __CUDA_ARCH__ >= 700
...
static __device__ __attribute__((always_inline)) double shfl_down( double r, int offset )
{
#if __CUDA_ARCH__ >= 700
...
struct Add
{
  ...
  static __device__ __attribute__((always_inline)) Value_type eval( Value_type x, Value_type y ) { return x+y; }
};
...
static __device__ __attribute__((always_inline)) Value_type warp_reduce( Value_type x )
{
...
template< int NUM_THREADS_PER_ITEM, typename Operator, typename Value_type >
static __device__ __attribute__((always_inline)) Value_type warp_reduce( volatile Value_type *smem, Value_type x )
{
